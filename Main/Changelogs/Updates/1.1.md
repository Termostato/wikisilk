...
This update brings a huge round of improvements, polish, and long-requested fixes to SilkCraft! Not only have we fine-tuned gameplay balance and squashed some stubborn bugs, but we've also introduced full compatibility with the beloved Actions & Stuff resource pack—no more conflicts or invisible mobs!
...
With that out of the way, here is the entire changelog for SilkCraft v1.1:
## Changes
...
- Added small log variants for End logs introduced in SilkCraft
...
- Implemented item groups for most SilkCraft items
- Sleeping raccoons now have visually closed eyes
- Most blocks in SilkCraft now utilize item proxies
- Enabled waterlogging for most SilkCraft blocks
- Crops and flowers from SilkCraft now break when in contact with water, aligning with vanilla behavior
- The **Ardent Mask** is now a wearable cosmetic item, exclusive to the Marketplace version
- Updated all foliage in SilkCraft to use the new vanilla cross model, matching Java Edition's cross model
...
- Removed SilkCraft's Falling Leaves
...
## Template Changes
- SilkCraft's world settings are now unlocked by default
- SilkCraft's default simulation distance is 4 chunks
...
- Resolved a conflict where *Actions & Stuff* made SilkCraft's vanilla mobs invisible
